            ...
            <div className="flex justify-between flex-col bg-white p-6 rounded-lg shadow-md min-h-80">
                ...
                <div className="flex justify-center space-x-6 h-full">
                    {rankings.slice(0, 3).map((user, index) => (
                        <div key={user.rank} className="text-center h-full content-end">
                            <div
                                className={`rounded-t-lg w-16 flex items-end justify-center pb-1 ${
                                    index === 1 ? "h-3/4 bg-yellow-400" : "h-2/4 bg-gray-300"
                                }`}
                                style={index === 2 ? { height: "25%" } : {}}
                            >
                                {user.rank}
                            </div>
                            <p className="font-semibold">{user.name}</p>
                            <p className="text-sm text-gray-600">Mark: {user.votes}</p>
                        </div>
                    ))}
                </div>
            </div>
                        ...
                        currentUsers.map((user, i) => (
                            <div key={i} className="flex items-center justify-between bg-gray-50 rounded p-3 shadow-sm">
                                <div className="flex items-center space-x-3">
                                    ...
                                    <div>
                                        <p className="font-semibold">{user.name}</p>
                                        <p className="text-sm text-gray-600">Avg stars: {user.rating.toFixed(1)}</p>
                                        <p className="text-xs text-gray-500">
                                            Progress: {user.progress} | Votes: {user.votes} | Rank: {user.rank}/{rankings.length}
                                        </p>
                                    </div>
                                </div>
