...
const Achievement = () => {
    ...
    return (
        <div className="flex flex-col items-center p-3 sm:p-4 md:p-6 bg-blue-900 bg-opacity-20 rounded-xl">
            <h1 className="text-2xl sm:text-3xl font-bold mb-4 sm:mb-6 md:mb-8 text-blue-300">Your Achievements</h1>
            ...
            <div className="w-full max-w-7xl mb-4 sm:mb-6">
                <div className="flex flex-col sm:flex-row items-center justify-between mb-4 gap-2 sm:gap-0">
                    <div className="text-amber-50 text-sm sm:text-base">
                        <span className="text-lg sm:text-xl font-semibold">{achievementsData.filter(a => a.unlocked).length}</span> of <span>{achievementsData.length}</span> achievements unlocked
                    </div>
                    <div className="bg-blue-800 bg-opacity-50 px-3 py-1 sm:px-4 sm:py-2 rounded-lg">
                        <span className="text-amber-50 font-semibold text-sm sm:text-base">Total Progress: </span>
                ...
                {paginatedAchievements.map((achievement, idx) => (
                    ...
                    >
                        {achievement.unlocked ? (
                            <span className="absolute top-2 sm:top-4 right-2 sm:right-4 bg-green-400 text-green-900 text-xs font-semibold px-2 py-1 rounded">UNLOCKED</span>
                        ) : (
                            <span className="absolute top-2 sm:top-4 right-2 sm:right-4 bg-gray-300 text-gray-800 text-xs font-semibold px-2 py-1 rounded">LOCKED</span>
                        )}
                        ...
                        <h2 className="text-lg sm:text-xl font-bold text-indigo-700 mb-2 text-center">{achievement.title}</h2>
                        <p className="text-gray-500 text-center text-sm sm:text-base mb-3 sm:mb-4">{achievement.description}</p>
